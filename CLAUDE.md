...
- **Routes** (`src/App.tsx`, all under `<Layout>`): `/` Landing, `/culture` CulturePractice, `/culture-engine` CultureEngine, `/ai-maestro` AIMaestro, `/coaching` AICoaching, `/workshops` Workshops, `/check/culture` CultureCheck, `/website` WebsiteRebuild, `/brief` Brief (leave-behind long-read, unlinked), `/insights` + `/insights/:slug` Insights/Insight, `*` NotFound. `WorkshopPrep.tsx` exists in `src/pages/` but is currently unrouted (taken down between workshop engagements, not deleted; see skill for the re-enable steps). Non-Landing routes are lazy-loaded (`React.lazy` + `Suspense`). `Layout` provides the sticky navbar + scroll manager; footers are per-page.
